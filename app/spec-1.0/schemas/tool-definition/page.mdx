...
- **`input_schema`** (required): A JSON Schema object that describes the input parameters for the tool.
  This schema supports standard JSON Schema validation, but excludes `$ref` and nested definitions/schemas for simplicity.

The `input_schema` MUST be a valid JSON Schema object and SHOULD contain a JSON-Schema `properties` object with descriptions for all properties, if the tool requires input parameters.

`input_schema` MAY be an empty object if the tool does not require input parameters.
